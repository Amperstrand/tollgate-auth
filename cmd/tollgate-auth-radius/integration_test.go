...
func TestE2ECashuReplayRejection(t *testing.T) {
	...
	first := processAuth(deps, token, mac1, "", "")
	if !first.Accept {
...
func TestE2ELNURLwAcceptFullFlow(t *testing.T) {
	...
	result := processAuth(deps, testtoken.LNURLwCode(), "aa:bb:cc:dd:ee:ff", "", "")
...
func TestE2ESplitTokenFullFlow(t *testing.T) {
	...
	result := processAuth(deps, rest178, "aa:bb:cc:dd:ee:ff", first200, "")
...
func TestE2EReconnectionFlow(t *testing.T) {
	...
	first := processAuth(deps, token, mac, "", "")
	if !first.Accept {
		t.Fatalf("first auth should Accept: %s", first.ReplyMessage)
	}

	second := processAuth(deps, "anything", mac, "", "")
	if !second.Accept {
...
func TestE2ESessionExpiredThenReauth(t *testing.T) {
	...
	first := processAuth(deps, token1, mac1, "", "")
	if !first.Accept {
		t.Fatalf("first auth should Accept: %s", first.ReplyMessage)
	}
	...
	reauth := processAuth(deps, token2, mac1, "", "")
	if !reauth.Accept {
